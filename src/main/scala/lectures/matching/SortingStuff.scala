...
object SortingStuff extends App {
  ...
  case class StuffBox(books: List[Book] = Nil,
    ...
    private def putStuffInRightBox(item: Stuff, stuffBox: StuffBox) = item match {
      case watches@Watches(_, cost) if cost > 1000 => stuffBox.copy(watches = watches :: stuffBox.watches)
      case book@Book(_, true) => stuffBox.copy(books = book :: stuffBox.books)
      case boots@Boots(brand, _) if brand == "Converse" || brand == "Adidas" => stuffBox.copy(boots = boots :: stuffBox.boots)
