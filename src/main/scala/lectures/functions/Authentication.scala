...
object Authentication extends App {
  ...
  val authByCard: PartialFunction[User, User] = {
    case user: CardUser if AuthenticationData.registeredCards(user.credentials) => user
  }

  val authByLP: PartialFunction[User, User] = {
    case user: LPUser if AuthenticationData.registeredLoginAndPassword(user.credentials) => user
  }
