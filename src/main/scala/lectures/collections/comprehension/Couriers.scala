...
object Address {
  def addresses(addressesCount: Int): List[Address] =
    (1 to addressesCount).toList.map(i => Address(s"$i$i$i"))
}
...
object CouriersWithComprehension extends App {
  ...
  def serveAddresses(addresses: List[Address], couriers: List[Courier]) = {
    val accum = couriers.filter(_ => traffic().degree < 5)
      .foldLeft(List[Address](), 0)( (x, courier) => {
        val newAddressList = addresses.slice(x._2, x._2 + courier.canServe)
        (x._1 ++ newAddressList, x._2 + courier.canServe)
      })
    accum._1
  }
